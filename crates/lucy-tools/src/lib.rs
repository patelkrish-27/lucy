...
impl ToolRegistry {
    ...
    pub fn definitions_filtered(&self, allowed: &HashSet<String>) -> Vec<Value> {
        self.tools.values().filter(|t|!t.name().starts_with("mcp_hyprfast_")||allowed.contains(t.name())).map(|t|serde_json::json!({"name":t.name(),"description":t.description(),"input_schema":t.parameters_schema()})).collect()
    }
    pub fn definitions_for_names(&self, allowed: &HashSet<String>) -> Vec<Value> {
        self.tools.values().filter(|t|allowed.contains(t.name())).map(|t|serde_json::json!({"name":t.name(),"description":t.description(),"input_schema":t.parameters_schema()})).collect()
    }
    ...
    pub async fn execute(&self, name: &str, input: Value, ctx: ToolContext) -> Result<Value> {
        ...
        let tool = self
            .get(name)
            .ok_or_else(|| anyhow!(LucyError::ToolNotFound(name.to_string())))?;
...
fn allowed_command(command: &str) -> bool {
    ...
    !blocked.iter().any(|x| c.contains(x))
}
...
impl Tool for ShellTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<Value> {
        ...
        if !allowed_command(command) {
            return Err(anyhow!(
                "command blocked by Lucy safety policy; set LUCY_ALLOW_DANGEROUS=1 only when explicitly requested"
            ));
        }
...
impl Tool for ListDirTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<Value> {
        ...
        while let Some(e) = rd.next_entry().await? {
            ...
            items.push(
                serde_json::json!({"name":e.file_name().to_string_lossy(),"directory":ft.is_dir()}),
            );
        }
...
impl Tool for SearchFilesTool {
    ...
    fn description(&self) -> &str {
        "Search text recursively with ripgrep."
    }
    fn parameters_schema(&self) -> Value {
        serde_json::json!({"type":"object","properties":{"query":{"type":"string"},"path":{"type":"string"}},"required":["query"]})
    }
    fn requires_approval(&self) -> bool {
        false
    }
...
impl Tool for GitTool {
    ...
    fn parameters_schema(&self) -> Value {
        serde_json::json!({"type":"object","properties":{"args":{"type":"array","items":{"type":"string"}}},"required":["args"]})
    }
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<Value> {
        let args = input
            .get("args")
...
impl Tool for EditFileTool {
    ...
    fn description(&self) -> &str {
        "Replace text in a UTF-8 text file (exact match). Set replace_all=true to replace every occurrence."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<Value> {
        let p = input
            .get("path")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!(LucyError::InvalidInput("path is required".into())))?;
        let old = input
            .get("old_string")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!(LucyError::InvalidInput("old_string is required".into())))?;
        let new = input
            .get("new_string")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!(LucyError::InvalidInput("new_string is required".into())))?;
        let replace_all = input
            .get("replace_all")
            .and_then(Value::as_bool)
            .unwrap_or(false);
        if old.is_empty() {
            return Err(anyhow!(LucyError::InvalidInput(
        ...
        if count > 1 && !replace_all {
            return Err(anyhow!(LucyError::InvalidInput(format!(
                "old_string matches {count} times; be more specific or set replace_all=true"
            ))));
        }
        let replacements = count as u64;
        let updated = if replace_all {
            content.replace(old, new)
        } else {
            content.replacen(old, new, 1)
        };
        let preview = edit_preview(&content, old, new, replace_all);
        tokio::fs::write(&path, &updated).await?;
        Ok(
            serde_json::json!({"path":path,"replacements":if replace_all{replacements}else{1},"diff":preview}),
        )
    }
}
fn edit_preview(orig: &str, old: &str, new: &str, replace_all: bool) -> String {
    ...
    if replace_all {
        let mut from = 0usize;
        while let Some(i) = orig[from..].find(old) {
            let idx = from + i;
            starts.push(orig[..idx].matches('\n').count());
            from = idx + old.len().max(1);
            if from >= orig.len() {
                break;
            }
        }
    }
    let covered: Vec<(usize, usize)> = if replace_all {
        starts
            .iter()
            .map(|&s| {
                let e = line_of_pos(orig, s, old);
                (s, e)
            })
            .collect()
    } else {
        let idx = orig.find(old).unwrap_or(0);
        let s = orig[..idx].matches('\n').count();
        vec![(s, line_of_pos(orig, s, old))]
    };
    fn line_of_pos(orig: &str, start_line: usize, old: &str) -> usize {
        let extra = old.matches('\n').count();
        let mut e = start_line + extra;
        if old.ends_with('\n') {
            e = e.saturating_sub(1);
        }
        let max = orig.lines().count().saturating_sub(1);
        e.min(max)
    }
    ...
    sorted.sort();
    for (s, e) in sorted {
        let s = s.min(n.saturating_sub(1));
        let e = e.min(n.saturating_sub(1)).max(s);
    ...
    for (cs, ce) in merged {
        let hs = cs.saturating_sub(3);
        let he = (ce + 3).min(n.saturating_sub(1));
        let slice_start = *offsets.get(cs).unwrap_or(&0);
        let slice_end = if ce + 1 < n {
            *offsets.get(ce + 1).unwrap_or(&orig.len())
        } else {
            orig.len()
        };
        let slice = orig.get(slice_start..slice_end).unwrap_or("");
        let new_slice = if replace_all {
            slice.replace(old, new)
        } else {
            slice.replacen(old, new, 1)
        };
        for l in hs..cs {
            if let Some(t) = lines.get(l) {
                out.push(format!(" {t}"));
            }
        }
        for l in cs..=ce {
...
impl Tool for GlobTool {
    ...
    fn description(&self) -> &str {
        "Find files by glob pattern (*, ?, ** segments). Recursive; skips .git, target and hidden directories; does not follow symlinked directories."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<Value> {
        let pattern = input
            .get("pattern")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!(LucyError::InvalidInput("pattern is required".into())))?;
        if pattern.is_empty() {
            return Err(anyhow!(LucyError::InvalidInput(
        ...
        let mut matched: Vec<String> = rels
            .into_iter()
            .filter(|r| glob_path_match(&segs, &r.split('/').collect::<Vec<_>>()))
            .collect();
        matched.sort();
        let truncated = matched.len() > 200;
        if truncated {
            matched.truncate(200);
        }
        Ok(serde_json::json!({"pattern":pattern,"path":p,"files":matched,"truncated":truncated}))
    }
}
...
fn glob_segment_match(pat: &str, text: &str) -> bool {
    ...
    let (mut star, mut mark): (Option<usize>, usize) = (None, 0usize);
    while ti < t.len() {
...
fn glob_path_match(pat: &[&str], path: &[&str]) -> bool {
    ...
    if pat[0] == "**" {
        for i in 0..=path.len() {
            if glob_path_match(&pat[1..], &path[i..]) {
                return true;
            }
        }
        return false;
