...
impl Tool for ShellTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<Value> {
        ...
        let mut child = tokio::process::Command::new("sh").arg("-lc").arg(command).current_dir(&working_dir).stdout(std::process::Stdio::piped()).stderr(std::process::Stdio::piped()).spawn()?;
        let stdout = child.stdout.take().ok_or_else(|| anyhow!("failed to capture stdout"))?; let stderr = child.stderr.take().ok_or_else(|| anyhow!("failed to capture stderr"))?;
        let limit = self.output_limit; let out_task = tokio::spawn(async move { read_limited(stdout, limit).await }); let err_task = tokio::spawn(async move { read_limited(stderr, limit).await });
