...
impl ToolRegistry {
    ...
    pub async fn execute(&self, name: &str, input: Value, ctx: ToolContext) -> Result<Value> { let tool = self.get(name).ok_or_else(|| anyhow!(LucyError::ToolNotFound(name.to_string())))?; tool.execute(input, ctx).await }
}
