...
impl HyprFastCatalog {
    ...
    pub async fn discover(config: McpServerConfig) -> Result<Self> {
        ...
        let tools = client
            .list_tools()
            .await
            .context("failed to discover HyprFast MCP tools")?;
    ...
    pub fn route(&self, prompt: &str) -> Route {
        ...
        let mut scored: Vec<(i32, &ToolCapability)> =
            self.tools.values().map(|t| (score(&text, t), t)).collect();
        scored.sort_by(|a, b| b.0.cmp(&a.0).then_with(|| a.1.name.cmp(&b.1.name)));
        let mut selected = HashSet::new();
        for (s, t) in scored.iter().take(8) {
            if *s > 0 {
                selected.insert(full_name(&t.name));
            }
        }
        if selected.is_empty() {
            for t in self.tools.values().filter(|t| !t.destructive).take(4) {
                selected.insert(full_name(&t.name));
            }
        }
        Route {
            candidates: selected.into_iter().collect(),
            strategy: strategy_for(&text),
            fast_path: is_fast_path(&text),
        }
    }
    pub fn route_domain(&self, category: &str, goal: &str) -> Route {
        let domain = match category.to_ascii_lowercase().as_str() {
            "browser" | "stagehand" | "hints" if !goal.is_empty() => {
                let _ = goal;
                Domain::Browser
            }
            _ => match category.to_ascii_lowercase().as_str() {
                "browser" => Domain::Browser,
                "stagehand" => Domain::Stagehand,
                "hints" => Domain::Hints,
        ...
        if domain != Domain::Unknown {
            let cands: Vec<String> = self
                .by_domain(domain)
                .iter()
                .map(|t| full_name(&t.name))
                .collect();
            if !cands.is_empty() {
                return Route {
                    candidates: cands,
                    strategy: strategy_for(&goal.to_ascii_lowercase()),
                    fast_path: false,
                };
            }
        }
    ...
    pub fn context_for(&self, route: &Route) -> String {
        ...
        out.push_str(&format!(
            "HyprFast catalog: {} tools, strategy={}, fast_path={}\n",
            self.tools.len(),
            route.strategy,
            route.fast_path
        ));
        ...
        for cand in &route.candidates {
            if let Some(cap) = self.capability_for_mcp_name(cand) {
                out.push_str(&format!(
                    "  {} [{}] - {}\n",
                    cand,
                    format!("{:?}", cap.domain),
...
fn score(text: &str, t: &ToolCapability) -> i32 {
    ...
    match t.domain {
        Domain::Browser => {
            if has(&[
                "browser", "website", "web", "url", "page", "tab", "youtube", "video", "music",
                "song", "play", "watch", "listen",
            ]) {
                s += 30
            }
        }
        ...
        Domain::Vision => {
            if has(&["screenshot", "screen", "see", "look", "visual"]) {
                s += 35
            }
        }
        Domain::Hints => {
            if has(&["click", "select", "press", "element"]) {
                s += 18
            }
        }
        Domain::Stagehand => {
            if has(&["complex", "workflow", "browser task"]) {
                s += 16
            }
        }
        _ => {}
    ...
    for c in &t.capabilities {
        s += match c {
            Capability::Click if has(&["click", "select", "press"]) => 28,
            Capability::Type if has(&["type", "write", "enter", "fill", "search"]) => 25,
            Capability::Navigate
                if has(&[
                    "open", "go to", "navigate", "visit", "url", "play", "watch", "listen",
                ]) =>
            {
                28
            }
            Capability::Window if has(&["window", "workspace", "move", "focus"]) => 28,
            Capability::Launch if has(&["open", "launch", "start", "play"]) => 26,
            Capability::Observe if has(&["see", "inspect", "find", "what", "screenshot"]) => 24,
            Capability::Extract if has(&["read", "extract", "get", "find"]) => 20,
            Capability::Batch if has(&["batch", "many", "multiple"]) => 35,
            Capability::Act if has(&["do", "execute", "perform", "act"]) => 12,
            _ => 0,
        };
    }
    if t.batchable && has(&["fast", "quick", "multiple", "many"]) {
        s += 18
    };
    if t.read_only && has(&["what", "check", "show", "see"]) {
        s += 8
    };
    if t.destructive && !has(&["close", "kill", "delete", "remove", "shutdown"]) {
        s -= 50
    };
    s
}
fn strategy_for(text: &str) -> String {
    if text.contains("screenshot") || text.contains("screen") {
        "direct-observe".into()
    } else if text.contains("click") || text.contains("select") || text.contains("press") {
        "hint-first".into()
    } else if text.contains("browser") || text.contains("website") || text.contains("url") {
        "browser-hint-ax-stagehand-vision".into()
    } else {
        "capability-first".into()
    }
}
...
fn classify(tool: &McpToolDefinition) -> ToolCapability {
    ...
    let domain = if has(&text, &["browser_", "browser "]) {
        Domain::Browser
    } else if has(&text, &["stagehand"]) {
        Domain::Stagehand
    } else if has(&text, &["hint_"]) {
        Domain::Hints
    } else if has(&text, &["screenshot", "ground", "vision"]) {
        Domain::Vision
        ...
        &text,
        &[
            "hypr", "desktop", "window", "pointer", "keyboard", "click_ui", "ui_",
        ],
    ) {
        Domain::Desktop
    } else {
    ...
    let read_only = matches!(domain, Domain::Vision)
        || has(
            &text,
            &["screenshot", "inspect", "get", "list", "find", "query"],
        );
    let destructive = has(
        &text,
        &[
    ...
    let operation = if read_only {
        ...
        Operation::Manage
    } else if has(&text, &["extract", "ground"]) {
        Operation::Transform
    } else {
        Operation::Act
    };
...
mod tests {
    ...
    fn categorizes_tools() {
        ...
        assert!(c.tools["act_batch"].batchable);
        assert_eq!(c.tools["task_init"].domain, Domain::Tasks);
    }
    #[test]
    fn routes_browser_click() {
        let c = HyprFastCatalog::from_tools(vec![
            tool("browser_click", "click browser element"),
            tool("screenshot", "capture desktop"),
            tool("desktop_window", "move workspace window"),
        ]);
        let r = c.route("click the browser button");
        assert!(r.candidates.iter().any(|x| x.contains("browser_click")));
        assert_eq!(r.strategy, "hint-first");
    }
    #[test]
    fn routes_play_song_to_browser_tools() {
        let c = HyprFastCatalog::from_tools(vec![
        ...
        let r = c.route("play sammi meri waar song");
        assert!(
            r.candidates.iter().any(|x| x.contains("browser_navigate")),
            "play-song must route to browser tools, got {:?}",
            r.candidates
        );
    }
}
