...
pub struct Environment { pub os:String,pub desktop:String,pub display_server:String,pub wayland:bool,pub hyprland:bool,pub hyprctl:bool,pub accessibility:bool,pub clipboard:bool,pub screenshot:bool }
impl Default for Environment { fn default()->Self{Self::detect()} }
impl Environment {
 pub fn detect()->Self{
  ...
  let hyprland=std::env::var_os("HYPRLAND_INSTANCE_SIGNATURE").is_some()||desktop_name.contains("hyprland")||std::env::var_os("HYPRLAND_CMD").is_some();
  ...
  let accessibility=std::env::var_os("AT_SPI_BUS_ADDRESS").is_some()||std::env::var("LUCY_ACCESSIBILITY").map(|v|v=="1"||v.eq_ignore_ascii_case("true")).unwrap_or(false);
 ...
 pub fn context(&self)->String{format!("OS={} desktop={} display_server={} wayland={} hyprland={} hyprctl={} accessibility={} clipboard={} screenshot={}",self.os,self.desktop,self.display_server,self.wayland,self.hyprland,self.hyprctl,self.accessibility,self.clipboard,self.screenshot)}
}
fn command_exists(command:&str)->bool{Command::new(command).arg("--version").output().map(|o|o.status.success()).unwrap_or(false)}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct HyprFastCatalog { pub tools:HashMap<String,ToolCapability>, #[serde(default)] pub environment:Environment }
impl HyprFastCatalog {
 ...
 pub fn route(&self,prompt:&str)->Route{let text=prompt.to_ascii_lowercase();let mut scored:Vec<(i32,&ToolCapability)>=self.tools.values().map(|t|(score(&text,t),t)).collect();scored.sort_by(|a,b|b.0.cmp(&a.0).then_with(||a.1.name.cmp(&b.1.name)));let mut selected=HashSet::new();for(s,t)in scored.iter().take(8){if *s>0{selected.insert(full_name(&t.name));}}if selected.is_empty(){for t in self.tools.values().filter(|t|!t.destructive).take(4){selected.insert(full_name(&t.name));}}Route{candidates:selected.into_iter().collect(),strategy:strategy_for(&text),fast_path:is_fast_path(&text)}}
 pub fn route_domain(&self,category:&str,goal:&str)->Route{let category_l=category.to_ascii_lowercase();let text=goal.to_ascii_lowercase();let domain=match category_l.as_str(){"browser"=>Domain::Browser,"stagehand"=>Domain::Stagehand,"hints"=>Domain::Hints,"vision"=>Domain::Vision,"clipboard"=>Domain::Clipboard,"excalidraw"=>Domain::Excalidraw,"tasks"=>Domain::Tasks,"desktop"=>Domain::Desktop,"system"=>Domain::System,_=>Domain::Unknown};if domain!=Domain::Unknown{let mut tools:Vec<&ToolCapability>=self.by_domain(domain);let explicit_launch=has(&text,&["launch browser","start browser","open browser","new browser window","new browser"]);if domain==Domain::Browser&&!explicit_launch{tools.retain(|t|!t.capabilities.contains(&Capability::Launch));}if domain==Domain::Desktop{tools.sort_by(|a,b|{let sa=desktop_score(&text,a,self);let sb=desktop_score(&text,b,self);sb.cmp(&sa).then_with(||a.name.cmp(&b.name))});}else{tools.sort_by(|a,b|score(&text,b).cmp(&score(&text,a)).then_with(||a.name.cmp(&b.name)));}let candidates=tools.into_iter().filter(|t|if domain==Domain::Desktop{desktop_score(&text,t,self)>0}else{score(&text,t)>0}).take(8).map(|t|full_name(&t.name)).collect::<Vec<_>>();if !candidates.is_empty(){let strategy=if domain==Domain::Browser&&has(&text,&["youtube","song","music","play","watch","listen","search"]){"direct-browser-navigation".into()}else if domain==Domain::Desktop{desktop_strategy(&text,self)}else{strategy_for(&text)};return Route{candidates,strategy,fast_path:false};}}self.route(goal)}
  ...
  pub fn planner_tool_set(&self, candidates:&[String], browser_ready:bool)->Vec<String>{
    ...
    let has_batch=candidates.iter().any(|n|self.capability_for_mcp_name(n).map(is_batch_tool).unwrap_or(false));
    for name in candidates{
      let Some(cap)=self.capability_for_mcp_name(name) else { out.push(name.clone()); continue; };
      if is_launch_tool(cap) {
        ...
        if matches!(cap.domain, Domain::Browser) && browser_ready { continue; }
        if matches!(cap.domain, Domain::Desktop) && browser_ready && cap.name.to_ascii_lowercase().contains("browser") { continue; }
      }
      if has_batch && matches!(cap.domain, Domain::Browser | Domain::Stagehand | Domain::Hints)
        && !cap.read_only && !is_batch_tool(cap) && !is_observation_tool(cap) {
    ...
    if !out.iter().any(|n|self.capability_for_mcp_name(n).map(|c|c.read_only||is_observation_tool(c)).unwrap_or(false)) {
      for name in candidates{
        if self.capability_for_mcp_name(name).map(|c|c.read_only||is_observation_tool(c)).unwrap_or(false)
          && !out.contains(name){ out.push(name.clone()); }
      }
    }
    out
  }
  ...
  pub fn batch_tools_for_domain(&self, domain:Domain)->Vec<&ToolCapability>{
    self.by_domain(domain).into_iter().filter(|c|is_batch_tool(c)).collect()
  }
  /// Compact `domain(count)` summary for the Router prompt (§4.1 — counts
  /// only, not full schemas).
  pub fn domain_summary_line(&self)->String{
    self.summary().into_iter().map(|(k,v)|format!("{k}({v})")).collect::<Vec<_>>().join(", ")
  }
}
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// (`execute_plan`, `act_batch`, ...). Detected via the Batch capability,
/// the catalog `batchable` flag, or well-known batch names.
pub fn is_batch_tool(cap:&ToolCapability)->bool{
  if cap.batchable || cap.capabilities.contains(&Capability::Batch){ return true; }
  let n=cap.name.to_ascii_lowercase();
  n.contains("execute_plan") || n.contains("act_batch") || n.contains("batch")
}
...
pub fn is_launch_tool(cap:&ToolCapability)->bool{
  if cap.capabilities.contains(&Capability::Launch){ return true; }
  let n=cap.name.to_ascii_lowercase();
  n.contains("browser_open") || n.contains("browser_launch") || n.contains("launch") || n.contains("open_application")
}
...
pub fn is_observation_tool(cap:&ToolCapability)->bool{
  if cap.read_only || cap.capabilities.contains(&Capability::Observe){ return true; }
  let n=cap.name.to_ascii_lowercase();
  n.contains("snapshot") || n.contains("extract") || n.contains("observe") || n.contains("get_ui") || n.contains("ui_tree") || n.contains("find_element") || n.contains("screenshot")
}
...
fn desktop_score(text:&str,t:&ToolCapability,env:&HyprFastCatalog)->i32{let n=t.name.to_ascii_lowercase();let mut s=0;if t.semantic{s+=20;}if env.environment.hyprland{if n.contains("window")||n.contains("space")||n.contains("workspace"){s+=25;}if n.contains("application")||n.contains("app_"){s+=15;}}if n.contains("find_element")||n.contains("ui_tree"){s+=80*if has(text,&["find","inspect","element","button","field"]){1}else{0}}if n.contains("press_button")||n.contains("click_element"){s+=90*if has(text,&["click","press","button"]){1}else{0}}if n.contains("menu"){s+=90*if has(text,&["menu","settings","preferences"]){1}else{0}}if n.contains("form"){s+=90*if has(text,&["form","fill","field"]){1}else{0}}if n.contains("application")||n.contains("app_"){s+=70*if has(text,&["open","launch","app"]){1}else{0}}if n.contains("window")||n.contains("space")||n.contains("display"){s+=70*if has(text,&["window","workspace","focus"]){1}else{0}}if n.contains("screenshot"){s+=80*if has(text,&["screenshot","screen","see","look"]){1}else{0}}if n.contains("clipboard"){s+=80*if has(text,&["clipboard","copy","paste"]){1}else{0}}if n.contains("type")||n.contains("key"){s+=60*if has(text,&["type","write","enter"]){1}else{0}}if n.contains("scroll")||n.contains("drag")||n.contains("mouse")||n.contains("pointer"){s+=40*if has(text,&["scroll","drag","mouse"]){1}else{0}}if t.read_only{s+=5}if t.destructive&&!has(text,&["close","delete","kill","shutdown","remove"]){s-=100}s}
fn desktop_strategy(text:&str,env:&HyprFastCatalog)->String{if env.environment.hyprland&&has(text,&["workspace","window","move","focus"]){"hyprland-semantic-desktop".into()}else if has(text,&["menu","settings","preferences"]){"semantic-menu".into()}else if has(text,&["form","fill","field","input"]){"semantic-form".into()}else if has(text,&["screenshot","screen","see","look"]){"visual-observe".into()}else if has(text,&["window","workspace","focus","app","application"]){"app-window-semantic".into()}else{"accessibility-first".into()}}
