...
pub fn build_waves(tasks: Vec<SubTask>) -> Vec<ExecutionWave> {
    ...
    while !pending.is_empty() {
        ...
        if wave.is_empty() {
            waves.push(ExecutionWave { tasks: deferred.into_iter().collect() });
            break;
        }

        for task in &wave { completed.insert(task.id.clone()); }
        pending = deferred;
...
pub fn has_dependency_cycle(tasks: &[SubTask]) -> bool {
    ...
    for task in tasks {
        for dep in &task.depends_on {
            if !ids.contains(dep.as_str()) { continue; }
            *indegree.entry(task.id.as_str()).or_default() += 1;
            edges.entry(dep.as_str()).or_default().push(task.id.as_str());
        }
    }

    let mut ready: VecDeque<&str> = indegree.iter().filter_map(|(id, degree)| (*degree == 0).then_some(*id)).collect();
    ...
    while let Some(id) = ready.pop_front() {
        ...
        if let Some(children) = edges.get(id) {
            for child in children {
                ...
                *degree -= 1;
                if *degree == 0 { ready.push_back(child); }
            }
        }
    }
    visited != ids.len()
...
mod tests {
    ...
    fn task(id: &str, category: &str, deps: &[&str]) -> SubTask {
        SubTask {
            ...
            success_condition: Some(format!("{id} succeeded")),
            required_observation: Some(format!("Observe state for {id}")),
            constraints: Vec::new(),
        }
    }
    ...
    fn independent_domains_share_a_wave() {
        let waves = build_waves(vec![task("browser-observe", "browser", &[]), task("desktop-observe", "desktop", &[])]);
    ...
    fn same_domain_is_serialized() {
        let waves = build_waves(vec![task("browser-a", "browser", &[]), task("browser-b", "browser", &[])]);
        assert_eq!(waves.len(), 2);
    }

    #[test]
    fn dependencies_create_ordered_waves() {
        let waves = build_waves(vec![task("observe", "browser", &[]), task("click", "browser", &["observe"]), task("report", "desktop", &["click"])]);
