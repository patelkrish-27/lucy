...
use lucy_core::{AgentEvent, InterruptSignal, SessionData, SessionId, ToolContext, TurnMessage, ToolCall, AssistantTurn, ToolResult, ExecutionMode};
...
struct Decomposition { #[serde(default)] subtasks: Vec<SubTask> }
#[derive(Debug, Clone, Deserialize)]
struct SubTask { id: String, goal: String, #[serde(default)] category: String, #[serde(default)] depends_on: Vec<String> }
#[derive(Debug, Clone, Deserialize)]
struct PlannedCommand { tool: String, arguments: Value, #[serde(default)] verify: Option<String> }

impl LucyRuntime {
 pub async fn new() -> Result<Self> {
  let provider=Arc::new(OpenAIProvider::from_env()?);
  let mut registry=default_registry();
  let hyprfast=match HyprFastCatalog::discover_default().await{Ok(c)=>{tracing::info!(tools=c.len(),"HyprFast MCP connected");let _=c.save().await;Some(c)},Err(e)=>{tracing::warn!(error=%e,"HyprFast MCP unavailable; continuing without desktop capabilities");None}};
  ...
  if hyprfast.is_some(){if let Err(e)=register_server(&mut registry,default_config()).await{tracing::warn!(error=%e,"failed to register HyprFast MCP tools")}}
  ...
  let session=match SessionData::load_from_file(&path).await{Ok(s)=>s,Err(e)=>{if path.exists(){return Err(e)}SessionData::new(SessionId::default())}};
  let registry=Arc::new(registry);let agent=Arc::new(Agent::new(provider.clone(),registry.clone()));
  Ok(Self{agent,registry,provider,session:Arc::new(Mutex::new(session)),session_path:path,interrupt:InterruptSignal::new(),working_dir:std::env::current_dir()?,hyprfast})
 }
 ...
 pub async fn submit(&self,prompt:String)->Result<mpsc::UnboundedReceiver<AgentEvent>> {
  self.interrupt.reset();let session=self.session.lock().await.clone();
  let route=self.route_hyprfast(&prompt);
  if let Some(r)=&route{tracing::debug!(strategy=%r.strategy,candidates=r.candidates.len(),fast_path=r.fast_path,"HyprFast route selected");}
  ...
  let(tx,rx)=mpsc::unbounded_channel();let session_store=self.session.clone();let path=self.session_path.clone();tokio::spawn(async move{let mut source=source;while let Some(event)=source.recv().await{if let AgentEvent::History{message}=&event{let mut session=session_store.lock().await;session.history.push(message.clone());session.updated_at=now();let _=session.save_to_file(&path).await;}let _=tx.send(event);}});Ok(rx)
 }
...
fn truncate_json(mut v:Value)->Value{let s=v.to_string();if s.len()<=MAX_CONTEXT_CHARS{return v}v=serde_json::json!({"truncated":true,"preview":&s[..s.char_indices().nth(MAX_CONTEXT_CHARS).map(|(i,_)|i).unwrap_or(s.len())]});v}
