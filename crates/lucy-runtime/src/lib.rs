...
use planner::{action_requires_verification, build_context, decide_next, plan_command, triage_request, truncate_json, validate_plan, validate_replanned_subtask, verification_subtask, DecisionKind};
use sessions::now;
...
pub struct LucyRuntime {
    ...
    store: SessionStore,
    #[allow(dead_code)] legacy_path: PathBuf,
...
impl LucyRuntime {
    pub async fn new() -> Result<Self> {
        ...
        let hyprfast = match HyprFastCatalog::discover(hf_cfg.clone()).await {
            Ok(c) => { tracing::info!(tools=c.len(), "HyprFast MCP connected"); let _=c.save().await; Some(c) }
            Err(e) => { tracing::warn!(error=%e, "HyprFast MCP unavailable; continuing without desktop capabilities"); None }
        };
        for server in load_config()? {
            if server.name.eq_ignore_ascii_case("hyprfast") { continue; }
            if let Err(e) = register_server(&mut registry, server.clone()).await { tracing::warn!(server=%server.name,error=%e,"MCP server unavailable"); }
        }
        ...
        let store=SessionStore::new(store_dir); let _=store.migrate_legacy_file(&legacy_path).await;
    ...
    async fn plan_and_execute(&self,prompt:String,history:Vec<TurnMessage>,route:Option<lucy_hyprfast::Route>)->Result<mpsc::UnboundedReceiver<AgentEvent>>{
        ...
        tokio::spawn(async move{let run=async{
            let _=tx.send(AgentEvent::History{message:TurnMessage::User(prompt.clone())});let _=tx.send(AgentEvent::Status{message:"Understanding your request…".into()});let _=tx.send(AgentEvent::Progress{message:"Understanding your request…".into()});
            ...
            let mut queue:VecDeque<SubTask>=triage.subtasks.into_iter().collect();let mut completed:HashMap<String,Value>=HashMap::new();let mut notes=Vec::new();let mut seen_actions=HashSet::<String>::new();let mut seen_failures=HashSet::<String>::new();let mut decisions=0usize;let mut replans=0usize;let mut actions=0usize;let mut done_count=0usize;let mut deferred=0usize;
            while let Some(subtask)=queue.pop_front(){
                if interrupt.is_set(){return Err(lucy_core::LucyError::Cancelled.into())} if actions>=MAX_ACTIONS{return Err(anyhow!("computer-operation action budget exhausted; stopping to prevent a loop"))}
                if !subtask.depends_on.iter().all(|id|completed.contains_key(id)){queue.push_back(subtask);deferred+=1;if deferred>=queue.len().max(1){return Err(anyhow!("execution plan is blocked by unresolved dependencies"))}continue} deferred=0;
                ...
                let must_verify=verify_actions&&planner_cfg.verify_state&&action_requires_verification(&catalog,&command.tool,&subtask.category,command.verify.is_some())&&!subtask.id.starts_with("verify-");if must_verify{let _=tx.send(AgentEvent::Status{message:"Verifying the result…".into()});let _=tx.send(AgentEvent::Progress{message:"Verifying the result…".into()});queue.push_front(verification_subtask(&catalog,&command.tool,&subtask.goal,&subtask.category,decisions+1));continue}
            ...
            let assistant=TurnMessage::Assistant(AssistantTurn{text:Some(text.clone()),tool_calls:Vec::new()});let _=tx.send(AgentEvent::History{message:assistant});let _=tx.send(AgentEvent::TextDelta{text});Ok::<(),anyhow::Error>(())
