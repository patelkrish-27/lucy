...
impl LucyRuntime{
 pub async fn new()->Result<Self>{
  ...
  let hyprfast=match HyprFastCatalog::discover_default().await{Ok(catalog)=>{tracing::info!(tools=catalog.len(),"HyprFast MCP connected");let _=catalog.save().await;Some(catalog)},Err(e)=>{tracing::warn!(error=%e,"HyprFast MCP unavailable; continuing without desktop capabilities");None}};
