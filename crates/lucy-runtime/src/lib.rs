...
impl LucyRuntime{
 ...
 pub async fn submit(&self,prompt:String)->Result<mpsc::UnboundedReceiver<AgentEvent>>{self.interrupt.reset();let session=self.session.lock().await.clone();let allowed:HashSet<String>=self.route_hyprfast(&prompt).map(|r|r.candidates.into_iter().collect()).unwrap_or_default();let route_info=self.route_hyprfast(&prompt);if let Some(route)=route_info{tracing::debug!(strategy=%route.strategy,candidates=route.candidates.len(),fast_path=route.fast_path,"HyprFast route selected");}let mut source=self.agent.execute_with_history_filtered(prompt,session.history,Some(self.working_dir.clone()),self.interrupt.clone(),allowed).await?;let(tx,rx)=mpsc::unbounded_channel();let session_store=self.session.clone();let path=self.session_path.clone();tokio::spawn(async move{while let Some(event)=source.recv().await{if let AgentEvent::History{message}=&event{let mut session=session_store.lock().await;session.history.push(message.clone());session.updated_at=now();let _=session.save_to_file(&path).await;}let _=tx.send(event);}});Ok(rx)}
