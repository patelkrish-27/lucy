...
pub async fn run_voice(stt: Option<Arc<GroqStt>>) -> anyhow::Result<()> {
    ...
    if let Some(rt) = runtime_opt.as_ref() {
        ...
        // Keep Settings UI in sync with the effective approval mode
        // (env vars like LUCY_AUTO_APPROVE can override the config file).
        app.config.approvals.mode = rt.approval_mode();
        if app.messages.is_empty() {
            if rt.is_auto() {
                app.status =
                    "Ready — auto-approve ON (no permission popups) — /auto off to ask".into();
            } else {
                    ...
                    Ok(event) => {
                        match event {
                            AgentEvent::Status { message } => {
                                ...
                                let phase = if low.contains("planning")
                                    || low.contains("understanding")
                                    || low.contains("preparing")
                                {
                                    "Thinking"
        ...
        while let Ok(result) = voice_rx.try_recv() {
            ...
            match result {
                ...
                Err(e) if e.contains("too short") || e.contains("no speech") => {
                    app.status =
                        format!("Speak after pressing {}, then press it again to send", "F2").into()
                }
                    ...
                    match key.code {
                        ...
                        KeyCode::Down => {
                            app.settings_selected =
                                (app.settings_selected + 1) % settings::ITEMS.len();
                        }
                        KeyCode::Left => {
                        ...
                        KeyCode::Char('s') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                            match app.config.save() {
                                Ok(()) => {
                                    // Live-apply approval mode so /auto and Settings stay in sync
                                    // without requiring a restart.
                                    if let Some(rt) = runtime_opt.as_ref() {
                                        rt.approval_state().set_mode(&app.config.approvals.mode);
                                    }
                                    app.status = "Settings saved — approvals applied live".into();
                                }
