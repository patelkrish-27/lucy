...
pub const ITEMS: [&str; 12] = [
    "Main model (OpenChat)","Main API Key — OpenChat","Main Endpoint — OpenChat",
    "Planner max subtasks","Planner max depth","HyprFast candidates","HyprFast batching","HyprFast parallel",
    "Verify actions","Voice model","Voice API Key","Voice hotkey"
];
...
pub fn draw(frame:&mut Frame<'_>, area:Rect, cfg:&LucyConfig, selected:usize) {
    ...
    let is_text_field = matches!(selected, 0|1|2|9|10|11);
    ...
    let display_value = match selected {
        ...
        2 => {
            if raw.is_empty() { "https://api.openchat.ai/v1 (default) — type endpoint".into() } else { raw.clone() }
        },
        _ if is_text_field => {
...
pub fn change(cfg:&mut LucyConfig, selected:usize, direction:i8) {
    match selected {
        3=>{let v=cfg.planner.max_subtasks as i32+direction as i32;cfg.planner.max_subtasks=v.clamp(1,256) as usize;}
        4=>{let v=cfg.planner.max_depth as i32+direction as i32;cfg.planner.max_depth=v.clamp(1,64) as usize;}
        5=>{let v=cfg.hyprfast.max_candidates as i32+direction as i32;cfg.hyprfast.max_candidates=v.clamp(1,68) as usize;}
        6=>cfg.hyprfast.batching=!cfg.hyprfast.batching,
        7=>cfg.hyprfast.parallel=!cfg.hyprfast.parallel,
        8=>cfg.hyprfast.verify_actions=!cfg.hyprfast.verify_actions,
        _=>{}
    }
}
pub fn edit_text(cfg:&mut LucyConfig,selected:usize,value:&str){
    ...
    match selected{
        0=>cfg.models.main=v,
        1=>cfg.models.main_api_key=if v.is_empty()||v=="(not set)"{None}else{Some(v)},
        2=>cfg.models.main_base_url=if v.is_empty()||v=="(not set)"||v.contains("(default)"){None}else{Some(v)},
        9=>cfg.voice.model=v,
        10=>cfg.voice.api_key=if v.is_empty()||v=="(not set)"{None}else{Some(v)},
        11=>cfg.voice.push_to_talk=v,
        _=>{}
    }
}
pub fn raw_value(cfg:&LucyConfig,selected:usize)->String{
    match selected{
        ...
        1=>cfg.models.main_api_key.clone().or_else(|| cfg.models.api_key.clone()).unwrap_or_default(),
        2=>cfg.models.main_base_url.clone().or_else(|| cfg.models.base_url.clone()).unwrap_or_default(),
        9=>cfg.voice.model.clone(),
        10=>cfg.voice.api_key.clone().unwrap_or_default(),
        11=>cfg.voice.push_to_talk.clone(),
        _=>String::new(),
    }
}
