...
pub const ITEMS: [&str; 11] = ["Main model","HyprFast command model","Planner max subtasks","Planner max depth","HyprFast candidates","HyprFast batching","HyprFast parallel","Verify actions","Voice model","Voice hotkey","Session resume"];
...
pub fn change(cfg:&mut LucyConfig, selected:usize, direction:i8) { match selected { 2=>{let v=cfg.planner.max_subtasks as i32+direction as i32;cfg.planner.max_subtasks=v.clamp(1,256) as usize;} 3=>{let v=cfg.planner.max_depth as i32+direction as i32;cfg.planner.max_depth=v.clamp(1,64) as usize;} 4=>{let v=cfg.hyprfast.max_candidates as i32+direction as i32;cfg.hyprfast.max_candidates=v.clamp(1,68) as usize;} 5=>cfg.hyprfast.batching=!cfg.hyprfast.batching, 6=>cfg.hyprfast.parallel=!cfg.hyprfast.parallel, 7=>cfg.hyprfast.verify_actions=!cfg.hyprfast.verify_actions, 10=>cfg.sessions.resume=!cfg.sessions.resume, _=>{} } }
pub fn edit_text(cfg:&mut LucyConfig,selected:usize,value:&str){match selected{0=>cfg.models.main=value.into(),1=>cfg.models.hyprfast_command=value.into(),8=>cfg.voice.model=value.into(),9=>cfg.voice.push_to_talk=value.into(),_=>{}}}
