...
pub const ITEMS: [&str; 17] = [
    "Main model (OpenChat)",
    "Cheap model (Gemini 3.5 Flash-Lite)",
    "Main API Key — OpenChat",
    "Main Endpoint — OpenChat",
    "Cheap API Key — Gemini",
    "Cheap Endpoint — Gemini",
...
pub fn draw(frame: &mut Frame<'_>, area: Rect, cfg: &LucyConfig, selected: usize) {
    ...
    frame.render_widget(
        List::new(items).block(list_block).style(popup_style),
        list_area,
    );
    ...
    let display_value = match selected {
        ...
        3 | 5 => {
            if raw.is_empty() {
                if selected == 3 {
                    "https://api.openchat.ai/v1 (default) — type endpoint".into()
                } else {
                    "https://generativelanguage.googleapis.com/v1beta/openai/ (Gemini)".into()
                }
            } else {
                raw.clone()
            }
        ...
        _ if is_text_field => {
            if raw.is_empty() {
                "(empty) — type to edit".into()
            } else {
                raw.clone()
            }
        }
        _ => {
            let approval_label = match cfg.approvals.mode.as_str() {
                "never" => "never (auto — never ask)",
                "always" => "always (ask for every tool)",
                _ => "write (default — ask for state-changing tools)",
            };
            let all_values = [
                ...
                mask(
                    &cfg.models
                        .main_api_key
                        .clone()
                        .or_else(|| cfg.models.api_key.clone()),
                ),
                cfg.models
                    .main_base_url
                    .clone()
                    .or_else(|| cfg.models.base_url.clone())
    ...
    let value_title = if is_text_field {
        " Value (typing edits immediately) "
    } else {
        " Value "
    };
    ...
    if is_text_field {
        let cursor_x = inner_detail.x
            + (inner_detail
                .width
                .saturating_sub(display_value.len() as u16 + 2))
                / 2
            + display_value.len() as u16
            + 1;
        let cursor_y = inner_detail.y + inner_detail.height / 2;
        frame.set_cursor_position((
            cursor_x.min(inner_detail.right().saturating_sub(1)),
            cursor_y,
        ));
    }
    let help = Paragraph::new(
        "↑/↓ select  ←/→ change  type to edit  Backspace deletes  Esc close  Ctrl+S save",
    )
    .alignment(Alignment::Center)
    .style(
        Style::default()
...
pub fn change(cfg: &mut LucyConfig, selected: usize, direction: i8) {
    match selected {
        6 => {
            let v = cfg.planner.max_subtasks as i32 + direction as i32;
            cfg.planner.max_subtasks = v.clamp(1, 256) as usize;
        }
        7 => {
            let v = cfg.planner.max_depth as i32 + direction as i32;
            cfg.planner.max_depth = v.clamp(1, 64) as usize;
        }
        8 => {
            let v = cfg.hyprfast.max_candidates as i32 + direction as i32;
            cfg.hyprfast.max_candidates = v.clamp(1, 68) as usize;
        }
        ...
        16 => {
            // Cycle approval mode: write -> never (auto) -> always -> write.
            // ←/→ or Enter all cycle forward; this is the hands-free switch.
            let _ = direction;
            cfg.approvals.mode = match cfg.approvals.mode.as_str() {
                "write" => "never".into(),
                "never" => "always".into(),
                _ => "write".into(),
            };
...
pub fn edit_text(cfg: &mut LucyConfig, selected: usize, value: &str) {
    ...
    match selected {
        ...
        2 => {
            cfg.models.main_api_key = if v.is_empty() || v == "(not set)" {
                None
            } else {
                Some(v)
            }
        ...
        3 => {
            cfg.models.main_base_url =
                if v.is_empty() || v == "(not set)" || v.contains("(default)") {
                    None
                } else {
                    Some(v)
                }
        ...
        4 => {
            cfg.models.cheap_api_key = if v.is_empty() || v == "(not set)" {
                None
            } else {
                Some(v)
            }
        ...
        13 => {
            cfg.voice.api_key = if v.is_empty() || v == "(not set)" {
                None
            } else {
                Some(v)
            }
