...
impl OpenAIProvider {
    ...
    pub fn from_config(cfg: &LucyConfig) -> Result<Self> {
        ...
        Ok(Self {
            client,
            config: cfg.clone(),
            model: Arc::new(std::sync::RwLock::new(cfg.models.main.clone())),
            usage: Arc::new(Mutex::new(TokenUsage::default())),
        })
    }
    pub fn from_env() -> Result<Self> {
        ...
        let model = std::env::var("OPENAI_MODEL")
            ...
            .unwrap_or_else(|_| cfg.models.main.clone());
        let mut cfg2 = cfg;
    ...
    async fn post_json(
                ...
                Err(e) => {
                    if attempt >= 3 {
                        return Err(e).context(format!(
                            "Model '{model}' request failed — endpoint {base_url} unreachable"
                        ));
                    }
                    if interrupt.is_set() {
                        return Err(LucyError::Cancelled.into());
                ...
                Ok(resp) => {
                    ...
                    if retryable {
                        if attempt >= 3 {
                            ...
                            return Err(anyhow!(
                                "Model '{model}' returned {status} from {base_url}: {body}",
                                status = status,
                                base_url = base_url,
        ...
        Err(anyhow!(
            "Model '{model}' request failed after retries — endpoint {base_url}"
        ))
    }
    pub async fn complete_json(
        ...
        let res = self
            .post_json(&url, &base_url, model, &api_key, &payload, &interrupt)
            .await?;
        ...
        let resp: Value = serde_json::from_str(&body)
            .context(format!("invalid JSON response from model '{model}'"))?;
        let _usage = Self::parse_usage(&resp);
        let content = resp["choices"][0]["message"]["content"]
            .as_str()
            .ok_or_else(|| anyhow!("Model '{model}' returned no content"))?;
        serde_json::from_str(content).or_else(|_| {
            let cleaned = content
                ...
                .trim_end_matches("```")
                .trim();
            serde_json::from_str(cleaned).context(format!("Model '{model}' returned invalid JSON"))
        })
    }
}
...
impl ModelProvider for OpenAIProvider {
    async fn run_turn(
        ...
        if !request.tools.is_empty() {
            payload["tools"]=json!(request.tools.iter().map(|t|json!({"type":"function","function":{"name":t["name"],"description":t["description"],"parameters":t["input_schema"]}})).collect::<Vec<_>>());
        }
        let api_key = self.api_key_for(&model)?;
        let base_url = self.base_url_for(&model);
        let url = format!("{}/chat/completions", base_url.trim_end_matches('/'));
