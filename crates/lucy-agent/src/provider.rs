...
impl OpenAIProvider {
    ...
    pub fn from_env()->Result<Self>{let cfg=LucyConfig::load().unwrap_or_default();Self::from_config(&cfg)}
    pub fn planner_model()->String{LucyConfig::load().map(|c|c.models.planner).unwrap_or_else(|_|"gpt-4o-mini".to_string())}
...
impl ModelProvider for OpenAIProvider {
    async fn run_turn(&self,request:ModelRequest,_events:mpsc::UnboundedSender<AgentEvent>,interrupt:InterruptSignal)->anyhow::Result<ModelTurn>{
        if interrupt.is_set(){return Err(LucyError::Cancelled.into());}
        let mut messages=Vec::new();messages.push(json!({"role":"system","content":"You are Lucy, a fast and capable AI computer-operation assistant. Prefer precise tool use, minimize unnecessary steps, and verify important actions."}));
        ...
        let mut payload=json!({"model":self.model,"messages":messages});if !request.tools.is_empty(){payload["tools"]=json!(request.tools.iter().map(|t|json!({"type":"function","function":{"name":t["name"],"description":t["description"],"parameters":t["input_schema"]}})).collect::<Vec<_>>());}
        let url=format!("{}/chat/completions",self.base_url.trim_end_matches('/'));let res=self.client.post(&url).bearer_auth(&self.api_key).json(&payload).send().await.context("OpenAI API request failed")?;let status=res.status();let body=res.text().await.context("failed to read response body")?;if !status.is_success(){return Err(anyhow!("OpenAI API returned {}: {}",status,body));}
