...
impl<P: ModelProvider> Agent<P> {
    ...
    pub async fn execute(&self, prompt: String, working_dir: Option<std::path::PathBuf>, interrupt: InterruptSignal) -> Result<mpsc::UnboundedReceiver<AgentEvent>> {
        ...
        tokio::spawn(async move {
            ...
            // Foundation hook: future model responses will yield structured ToolCall values.
            // Keeping execution behind ToolRegistry prevents the UI from owning side effects.
            let _ = tools;
            let _ = working_dir;
            let _ = tx.send(AgentEvent::Done);
        });
