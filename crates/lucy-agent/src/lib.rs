...
impl<P: ModelProvider + 'static> Agent<P> {
    ...
    pub fn with_approval(self, gate: ApprovalGate) -> Self {
        Self {
    ...
    pub async fn execute(
        ...
        self.execute_with_history(prompt, Vec::new(), working_dir, interrupt)
            .await
    }
    pub async fn execute_with_history(
        ...
        self.execute_with_history_filtered(prompt, history, working_dir, interrupt, HashSet::new())
            .await
    }
    pub async fn execute_with_history_filtered(
        &self,
        prompt: String,
        history: Vec<TurnMessage>,
        working_dir: Option<std::path::PathBuf>,
        interrupt: InterruptSignal,
        allowed_hyprfast: HashSet<String>,
    ) -> Result<mpsc::UnboundedReceiver<AgentEvent>> {
        let (provider, tools, approver) = (
            self.provider.clone(),
            self.tools.clone(),
            self.approver.clone(),
        );
...
async fn run_loop<P: ModelProvider>(
    ...
    let user = TurnMessage::User(prompt.clone());
    history.push(user.clone());
    let _ = tx.send(AgentEvent::History { message: user });
    let mut turns = 0;
    loop {
        if interrupt.is_set() {
        ...
        if turns >= MAX_TOOL_TURNS {
            return Err(anyhow::anyhow!(
                "tool-turn limit reached ({MAX_TOOL_TURNS})"
            ));
        }
        turns += 1;
        let _ = tx.send(AgentEvent::Status {
            message: format!("Planning turn {turns}…"),
        });
        let request = ModelRequest {
            session_id: session_id.clone(),
            prompt: prompt.clone(),
            history: history.clone(),
            tools: tools.definitions_filtered(&allowed_hyprfast),
        };
        let turn = provider
            .run_turn(request, tx.clone(), interrupt.clone())
            .await?;
                ...
                if gate.needs_approval(&call.name, tools.requires_approval(&call.name)) {
                    match gate.ask(&call.id, &call.name, &call.input).await {
                        ...
                        ApprovalDecision::Deny => {
                            ...
                            let tool = TurnMessage::Tool(ToolResult {
                                call_id: call.id,
                                name: call.name,
                                output,
                                is_error: true,
