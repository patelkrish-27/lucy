...
/// Groq Whisper adapter implementing ADK's unified STT contract.
///
/// ADK Audio currently ships Whisper/OpenAI, Deepgram, AssemblyAI and Gemini
/// STT providers, but not Groq's Whisper-compatible endpoint. Lucy therefore
/// keeps Groq as its configured provider while making it consume/produce ADK
/// `AudioFrame`/`Transcript` types.
#[derive(Debug, Clone)]
struct AdkGroqStt {
    client: Client,
...
impl AdkGroqStt {
    ...
    async fn transcribe_frame(
        ...
        audio: &AudioFrame,
        opts: &SttOptions,
    ) -> std::result::Result<Transcript, AudioError> {
        let wav = encode(audio, AudioFormat::Wav)
            .map_err(|e| AudioError::Codec(e.to_string()))?;
        if wav.len() > MAX_UPLOAD_BYTES {
            return Err(AudioError::Stt {
                provider: "groq".into(),
                message: "audio exceeds Groq's 25 MB upload limit".into(),
            });
        }
        ...
        let response = self
            ...
            .map_err(|e| AudioError::Stt {
                provider: "groq".into(),
                message: format!("Groq STT request failed: {e}"),
            })?;

        let status = response.status();
        let body = response.text().await.map_err(|e| AudioError::Stt {
            provider: "groq".into(),
            message: format!("failed to read Groq response: {e}"),
        })?;
        ...
        if status != StatusCode::OK {
            ...
            return Err(AudioError::Stt {
                provider: "groq".into(),
                message: format!("Groq STT returned {status}: {message}"),
            });
        }
        ...
        let text = serde_json::from_str::<TranscriptionResponse>(&body)
            .map_err(|e| AudioError::Stt {
                provider: "groq".into(),
                message: format!("Groq returned an invalid transcription response: {e}"),
            })?
            .text
            .trim()
            .to_owned();
        ...
        Ok(Transcript {
            text,
            language_detected: self.config.language.clone(),
            ..Default::default()
        })
    }
}
...
impl SttProvider for AdkGroqStt {
    async fn transcribe(
        ...
        self.transcribe_frame(audio, opts).await
    }

    async fn transcribe_stream(
        &self,
        mut audio: std::pin::Pin<Box<dyn futures::Stream<Item = AudioFrame> + Send>>,
        opts: &SttOptions,
    ) -> std::result::Result<
        std::pin::Pin<Box<dyn futures::Stream<Item = std::result::Result<Transcript, AudioError>> + Send>>,
        AudioError,
    > {
        ...
        if frames.is_empty() {
            return Err(AudioError::Stt {
                provider: "groq".into(),
                message: "audio stream is empty".into(),
            });
        }
...
pub struct GroqStt {
    config: GroqSttConfig,
    provider: Arc<AdkGroqStt>,
}

impl GroqStt {
    pub fn new(config: GroqSttConfig) -> Result<Self> {
        let provider = Arc::new(AdkGroqStt::new(config.clone())?);
        Ok(Self { config, provider })
    }

    pub fn from_config(cfg: &LucyConfig) -> Result<Self> {
        Self::new(GroqSttConfig::from_config(cfg)?)
    }
    ...
    pub async fn transcribe_file<P: AsRef<Path>>(&self, path: P) -> Result<String> {
        ...
        let filename = path
            ...
            .and_then(|v| v.to_str())
            .unwrap_or("audio.wav");
        self.transcribe_bytes(audio, filename, mime_for_filename(filename))
            .await
    }
    ...
    pub async fn transcribe_bytes(
        ...
        if audio.len() > MAX_UPLOAD_BYTES {
            return Err(anyhow!("audio exceeds Groq's 25 MB upload limit"));
        }
        ...
        let form = multipart::Form::new()
            ...
            .text(
                "prompt",
                self.config.prompt.as_deref().unwrap_or(DEFAULT_PROMPT).to_owned(),
            );
        let response = self
            .provider
            .client
        ...
        Ok(serde_json::from_str::<TranscriptionResponse>(&body)
    ...
    /// Start an ADK desktop-audio hold-to-talk capture.
    pub fn start_hold(&self) -> Result<HoldCapture> {
        HoldCapture::start()
    }
    ...
    fn stt_options(&self) -> SttOptions {
        SttOptions {
            ...
            model_hint: Some(self.config.model.clone()),
            ..Default::default()
        }
    }
}
...
/// ADK Audio boundary. It can later be swapped for a neural VAD without changing
/// the TUI or STT API.
...
impl VadProcessor for LucyEnergyVad {
    fn is_speech(&self, frame: &AudioFrame) -> bool {
        rms_i16(frame.samples()) >= END_THRESHOLD
    }
...
/// Active ADK desktop-audio push-to-talk recording.
...
fn input_device_id() -> Result<String> {
    if let Ok(id) = std::env::var("LUCY_AUDIO_INPUT_DEVICE") {
        if !id.trim().is_empty() {
            return Ok(id);
        }
    }
...
fn adk_capture_config() -> CaptureConfig {
    CaptureConfig::default()
}

fn collected_frame(frames: &[AudioFrame]) -> Result<AudioFrame> {
    if frames.is_empty() {
        return Err(anyhow!("no audio captured"));
    }
    Ok(lucy_adk::adk_audio::merge_frames(frames))
}
...
pub fn has_speech(samples: &[i16], sample_rate: u32, channels: u16) -> bool {
    ...
    let window_size = ((sample_rate as usize) * ch / 10).max(1);
...
mod tests {
    ...
    #[test]
    fn adk_vad_preserves_lucy_threshold() {
        ...
        assert!(LucyEnergyVad::default().is_speech(&frame));
    }
}
