...
#[derive(Debug, Deserialize)] struct GroqError { message: Option<String> }
#[derive(Debug, Deserialize)] struct TranscriptionResponse { text: String }
impl GroqStt {
    pub fn new(config: GroqSttConfig) -> Result<Self> { let client=Client::builder().timeout(config.timeout).build().context("failed to create HTTP client")?;Ok(Self{client,config}) }
    ...
    pub async fn transcribe_file<P:AsRef<Path>>(&self,path:P)->Result<String>{let path=path.as_ref();let audio=tokio::fs::read(path).await.with_context(||format!("failed to read audio file: {}",path.display()))?;let filename=path.file_name().and_then(|v|v.to_str()).unwrap_or("audio.wav");self.transcribe_bytes(audio,filename,mime_for_filename(filename)).await}
    pub async fn transcribe_bytes(&self,audio:Vec<u8>,filename:&str,mime:&str)->Result<String>{
        ...
        let prompt=self.config.prompt.as_deref().unwrap_or("User voice commands for computer assistant");
    ...
    pub async fn listen_once(&self)->Result<String>{let(audio,sample_rate,channels)=tokio::task::spawn_blocking(capture_utterance).await.context("microphone task failed")??;let wav=encode_wav(&audio,sample_rate,channels)?;self.transcribe_bytes(wav,"lucy-live.wav","audio/wav").await}
    ...
    /// Start a hold-to-talk capture. Audio records until `finish_hold` (key
    /// release) or `cancel_hold`. Unlike `listen_once` there is no silence
    /// timeout — release always ends the recording.
    pub fn start_hold(&self)->Result<HoldCapture>{HoldCapture::start()}
    pub async fn finish_hold(&self,cap:HoldCapture)->Result<String>{
        let (samples,sample_rate,channels)=cap.finish();
        // Require ~0.35s of audio so accidental taps don't hit the network.
        let min_samples=((sample_rate as usize)*(channels as usize)*35)/100;
        if samples.len()<min_samples{return Err(anyhow!("too short — hold the hotkey while speaking, then release"));}
        if !has_speech(&samples,sample_rate,channels){return Err(anyhow!("no speech detected"));}
        let wav=encode_wav(&samples,sample_rate,channels)?;
        self.transcribe_bytes(wav,"lucy-live.wav","audio/wav").await
    }
}
/// Active hold-to-talk recording. Keep the `cpal::Stream` alive while the
/// hotkey is held; dropping it stops the microphone.
pub struct HoldCapture{stream:Option<cpal::Stream>,samples:Arc<Mutex<Vec<i16>>>,sample_rate:u32,channels:u16}
impl HoldCapture{
    fn start()->Result<Self>{
        ...
        Ok(Self{stream:Some(stream),samples,sample_rate,channels})
    }
    fn finish(mut self)->(Vec<i16>,u32,u16){
        drop(self.stream.take());
        let samples=self.samples.lock().map(|g|g.clone()).unwrap_or_default();
        (samples,self.sample_rate,self.channels)
    }
}
fn capture_utterance()->Result<(Vec<i16>,u32,u16)>{let host=cpal::default_host();let device=host.default_input_device().ok_or_else(||anyhow!("no default microphone found"))?;let supported=device.default_input_config().context("failed to get microphone configuration")?;let sample_rate=supported.sample_rate().0;let channels=supported.channels();let config:cpal::StreamConfig=supported.clone().into();let samples:Arc<Mutex<Vec<i16>>>=Arc::new(Mutex::new(Vec::new()));let samples_cb=samples.clone();let err_fn=|err|eprintln!("Lucy microphone error: {err}");let stream=match supported.sample_format(){cpal::SampleFormat::F32=>device.build_input_stream(&config,move|data:&[f32],_|push_f32(data,&samples_cb),err_fn,None),cpal::SampleFormat::I16=>device.build_input_stream(&config,move|data:&[i16],_|push_i16(data,&samples_cb),err_fn,None),cpal::SampleFormat::U16=>device.build_input_stream(&config,move|data:&[u16],_|push_u16(data,&samples_cb),err_fn,None),format=>return Err(anyhow!("unsupported microphone sample format: {format:?}")),}.context("failed to build microphone stream")?;stream.play().context("failed to start microphone")?;const START_THRESHOLD:f32=0.012;const END_THRESHOLD:f32=0.008;const START_WINDOW:Duration=Duration::from_millis(120);const SILENCE_AFTER_SPEECH:Duration=Duration::from_millis(750);const MAX_UTTERANCE:Duration=Duration::from_secs(20);const POLL:Duration=Duration::from_millis(40);let started=Instant::now();let mut speech_started=false;let mut last_voice=Instant::now();let mut seen_samples=0usize;loop{std::thread::sleep(POLL);let current={let guard=samples.lock().map_err(|_|anyhow!("microphone buffer poisoned"))?;guard.clone()};if current.len()>seen_samples{let recent=&current[seen_samples..];let rms=rms_i16(recent);if !speech_started&&rms>=START_THRESHOLD&&started.elapsed()>=START_WINDOW{speech_started=true;last_voice=Instant::now();}else if speech_started&&rms>=END_THRESHOLD{last_voice=Instant::now();}seen_samples=current.len();}if speech_started&&last_voice.elapsed()>=SILENCE_AFTER_SPEECH{break;}if started.elapsed()>=MAX_UTTERANCE{break;}}drop(stream);let result=samples.lock().map_err(|_|anyhow!("microphone buffer poisoned"))?.clone();if !speech_started||result.is_empty(){return Err(anyhow!("no speech detected"));}Ok((result,sample_rate,channels))}
...
pub fn has_speech(samples: &[i16], sample_rate: u32, channels: u16) -> bool {
    if samples.is_empty() { return false; }
    let ch = channels.max(1) as usize;
    let window_size = ((sample_rate as usize) * ch / 10).max(1); // 100ms window
...
fn encode_wav(samples:&[i16],sample_rate:u32,channels:u16)->Result<Vec<u8>>{let mut cursor=std::io::Cursor::new(Vec::new());let spec=hound::WavSpec{channels,sample_rate,bits_per_sample:16,sample_format:hound::SampleFormat::Int};let mut writer=hound::WavWriter::new(&mut cursor,spec).context("failed to create WAV")?;for &sample in samples{writer.write_sample(sample).context("failed to encode WAV")?;}writer.finalize().context("failed to finalize WAV")?;Ok(cursor.into_inner())}
...
mod tests {
    ...
    #[test]
    fn detects_silence_correctly() {
        // Zero audio
        let silence = vec![0i16; 16000];
        assert!(!has_speech(&silence, 16000, 1));

        // Low ambient noise (~0.001 RMS)
        let noise: Vec<i16> = (0..16000).map(|i| if i % 2 == 0 { 40 } else { -40 }).collect();
        assert!(!has_speech(&noise, 16000, 1));
    }

    #[test]
    fn detects_speech_signal() {
        // Clear speech burst (~0.05 RMS for 400ms)
        let mut audio = vec![0i16; 16000];
        for i in 4000..10400 {
            audio[i] = ((i as f32 * 0.1).sin() * 2000.0) as i16;
        }
        assert!(has_speech(&audio, 16000, 1));
    }
}
