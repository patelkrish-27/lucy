...
impl GroqStt {
    ...
    pub async fn transcribe_bytes(
        ...
        let prompt = self
            ...
            .prompt
            .as_deref()
            .unwrap_or("User voice commands for computer assistant");
    ...
    pub async fn listen_once(&self) -> Result<String> {
        let (audio, sample_rate, channels) = tokio::task::spawn_blocking(capture_utterance)
            .await
            .context("microphone task failed")??;
        let wav = encode_wav(&audio, sample_rate, channels)?;
        self.transcribe_bytes(wav, "lucy-live.wav", "audio/wav")
            .await
    }
    ...
    pub async fn finish_hold(&self, cap: HoldCapture) -> Result<String> {
        ...
        if samples.len() < min_samples {
            return Err(anyhow!(
                "too short — hold the hotkey while speaking, then release"
            ));
        }
        if !has_speech(&samples, sample_rate, channels) {
    ...
    fn start() -> Result<Self> {
        ...
        let stream = match supported.sample_format() {
            cpal::SampleFormat::F32 => device.build_input_stream(
                &config,
                move |data: &[f32], _| push_f32(data, &samples_cb),
                err_fn,
                None,
            ),
            cpal::SampleFormat::I16 => device.build_input_stream(
                &config,
                move |data: &[i16], _| push_i16(data, &samples_cb),
                err_fn,
                None,
            ),
            cpal::SampleFormat::U16 => device.build_input_stream(
                &config,
                move |data: &[u16], _| push_u16(data, &samples_cb),
                err_fn,
                None,
            ),
...
fn capture_utterance() -> Result<(Vec<i16>, u32, u16)> {
    ...
    loop {
        ...
        let current = {
            let guard = samples
                .lock()
                .map_err(|_| anyhow!("microphone buffer poisoned"))?;
            guard.clone()
        };
    ...
    let result = samples
        ...
        .map_err(|_| anyhow!("microphone buffer poisoned"))?
        .clone();
    if !speech_started || result.is_empty() {
        return Err(anyhow!("no speech detected"));
    }
...
fn push_f32(data: &[f32], dst: &Arc<Mutex<Vec<i16>>>) {
    if let Ok(mut out) = dst.lock() {
        out.extend(data.iter().map(|&x| (x.clamp(-1.0, 1.0) * 32767.0) as i16));
    }
}
fn push_i16(data: &[i16], dst: &Arc<Mutex<Vec<i16>>>) {
    if let Ok(mut out) = dst.lock() {
        out.extend_from_slice(data);
    }
}
fn push_u16(data: &[u16], dst: &Arc<Mutex<Vec<i16>>>) {
    if let Ok(mut out) = dst.lock() {
        out.extend(data.iter().map(|&x| (x as i32 - 32768) as i16));
    }
}
