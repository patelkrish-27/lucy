...
#[derive(Debug, Clone, Serialize, Deserialize)] #[serde(default)] pub struct ModelConfig { pub main:String, pub planner:String, pub base_url:Option<String> }
...
impl LucyConfig {
 ...
 fn apply_env(&mut self)->Result<()>{if let Some(v)=env::var_os("OPENAI_MODEL"){self.models.main=v.to_string_lossy().into_owned();}if let Some(v)=env::var_os("LUCY_PLANNER_MODEL"){self.models.planner=v.to_string_lossy().into_owned();}if let Some(v)=env::var_os("OPENAI_BASE_URL"){self.models.base_url=Some(v.to_string_lossy().into_owned());}if let Some(v)=env::var_os("LUCY_STT_MODEL"){self.voice.model=v.to_string_lossy().into_owned();}if let Some(v)=env::var_os("LUCY_STT_LANGUAGE"){self.voice.language=Some(v.to_string_lossy().into_owned());}if let Some(v)=env::var_os("LUCY_HYPRFAST_MAX_CANDIDATES"){self.hyprfast.max_candidates=v.to_string_lossy().parse()?;}Ok(())}
}
...
pub fn doctor()->Vec<(&'static str,bool,String)>{match LucyConfig::load(){Ok(c)=>vec![("Config",true,LucyConfig::path().map(|p|p.display().to_string()).unwrap_or_default()),("Planner",true,c.models.planner),("HyprFast",command_exists(&c.hyprfast.command),c.hyprfast.command)],Err(e)=>vec![("Config",false,e.to_string())]}}
