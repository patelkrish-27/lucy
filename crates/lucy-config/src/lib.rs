...
pub struct ModelConfig {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub main_base_url: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl LucyConfig {
    ...
    fn apply_env(&mut self) -> Result<()> {
        if let Some(v) = env::var_os("OPENAI_MODEL") {
            self.models.main = v.to_string_lossy().into_owned();
        }
        if let Some(v) = env::var_os("LUCY_PLANNER_MODEL") {
            self.models.hyprfast_command = v.to_string_lossy().into_owned();
        }
        if let Some(v) = env::var_os("LUCY_HYPRFAST_COMMAND_MODEL") {
            self.models.hyprfast_command = v.to_string_lossy().into_owned();
        }
        if let Some(v) = env::var_os("GEMINI_MODEL") {
            self.models.hyprfast_command = v.to_string_lossy().into_owned();
        }
        ...
        if self.models.main_base_url.is_none() {
            ...
            {
                if !v.trim().is_empty() {
                    self.models.main_base_url = Some(v);
                }
            }
        }
        if self.models.cheap_base_url.is_none() {
            if let Ok(v) = env::var("GEMINI_BASE_URL")
                .or_else(|_| env::var("LUCY_CHEAP_BASE_URL"))
                .or_else(|_| env::var("CHEAP_BASE_URL"))
                .or_else(|_| env::var("LUCY_HYPRFAST_BASE_URL"))
            {
                if !v.trim().is_empty() {
                    self.models.cheap_base_url = Some(v);
                }
            }
        }
        ...
        if self.models.main_api_key.is_none() {
            for key in [
                ...
                if let Ok(v) = env::var(key) {
                    if !v.trim().is_empty() {
                        self.models.main_api_key = Some(v);
                        break;
                    }
                }
            }
        ...
        if self.models.cheap_api_key.is_none() {
            for key in [
                ...
                "GEMINI_API_KEY",
                "CHEAP_API_KEY",
                "LUCY_HYPRFAST_API_KEY",
                "HYPRFAST_API_KEY",
            ] {
                if let Ok(v) = env::var(key) {
                    if !v.trim().is_empty() {
                        self.models.cheap_api_key = Some(v);
                        break;
                    }
                }
            }
        ...
        // Approval mode: `never` = auto-approve everything (no permission
        // popups), `write` = ask for state-changing tools (default),
        // `always` = ask for every tool. `LUCY_AUTO_APPROVE=1` is shorthand
        // for `LUCY_APPROVALS_MODE=never` so voice users can opt into
        // hands-free computer control without editing the config file.
        if let Ok(v) = env::var("LUCY_APPROVALS_MODE") {
            let m = v.trim().to_ascii_lowercase();
            if !m.is_empty() {
                self.approvals.mode = match m.as_str() {
                    "auto" | "off" | "on" => "never".into(),
                    "default" | "ask" => "write".into(),
                    other => other.into(),
                };
            }
        }
        if let Ok(v) = env::var("LUCY_AUTO_APPROVE").or_else(|_| env::var("LUCY_AUTO")) {
            let on = matches!(
                v.trim().to_ascii_lowercase().as_str(),
                "1" | "true" | "yes" | "on" | "y"
            );
            if on {
                self.approvals.mode = "never".into();
            }
        }
        Ok(())
    }
    ...
    pub fn api_key_for(&self, model: &str) -> Option<String> {
        if model == self.models.main {
            self.main_api_key()
        } else if model == self.models.hyprfast_command {
            self.cheap_api_key()
        } else {
            self.llm_api_key()
        }
...
pub fn doctor() -> Vec<(&'static str, bool, String)> {
    let config_status = match LucyConfig::load() {
        Ok(c) => {
            ...
            let main_endpoint = c
                .main_base_url()
                .unwrap_or_else(|| "https://api.openchat.ai/v1".into());
            let cheap_endpoint = c.cheap_base_url().unwrap_or_else(|| {
            ...
            let cheap_reach = reachability(&cheap_endpoint);
            let main_detail = if main_ok {
                ...
                "missing — Settings > Main API Key (OpenChat) or env OPENCHAT_API_KEY / LUCY_MAIN_API_KEY".into()
            };
            let cheap_detail = if cheap_ok {
                format!(
                    "set {} — endpoint {}",
                    mask(cheap_key.unwrap()),
                    cheap_endpoint
                )
            } else {
                "missing — Settings > Cheap API Key (Gemini) or env GEMINI_API_KEY / LUCY_CHEAP_API_KEY".into()
            };
            vec![
                (
                    "Config",
                ...
                (
                    "Main model",
                    true,
                    format!("{} (OpenChat)", c.models.main.clone()),
                ),
                (
                    "Main Endpoint",
                    true,
                ...
                (
                    ...
                    format!(
                        ...
                        c.models.hyprfast_command.clone()
                    ),
                ),
                (
                    "Cheap Endpoint",
                    true,
                    format!("{cheap_endpoint}{cheap_reach}"),
                ),
