...
impl InterruptSignal {
    pub fn new() -> Self {
        Self { flag: Arc::new(AtomicBool::new(false)), epoch: Arc::new(AtomicU64::new(0)), notify: Arc::new(Notify::new()) }
    }
...
pub enum AgentEvent {
    ...
    ToolFinished { id: String, output: Value },
...
impl ToolContext {
    pub fn resolve_path(&self, path: &Path) -> PathBuf {
        if path.is_absolute() { path.to_path_buf() } else if let Some(base) = &self.working_dir { base.join(path) } else { path.to_path_buf() }
    }
}
...
pub trait ModelProvider: Send + Sync {
    async fn run(&self, prompt: &str, events: mpsc::UnboundedSender<AgentEvent>, interrupt: InterruptSignal) -> anyhow::Result<()>;
}
