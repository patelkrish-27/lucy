...
#[derive(Clone)]
struct FileMakeWriter{file:Arc<std::sync::Mutex<std::fs::File>>}
impl<'a> tracing_subscriber::fmt::MakeWriter<'a> for FileMakeWriter{
    type Writer=TeeWriter;
    fn make_writer(&'a self)->Self::Writer{TeeWriter{stderr:std::io::stderr(),file:self.file.clone()}}
}
struct TeeWriter{stderr:std::io::Stderr,file:Arc<std::sync::Mutex<std::fs::File>>}
impl std::io::Write for TeeWriter{
    fn write(&mut self,buf:&[u8])->std::io::Result<usize>{
        if let Ok(mut f)=self.file.lock(){let _=std::io::Write::write_all(&mut *f,buf);let _=std::io::Write::flush(&mut *f);}
        std::io::Write::write(&mut self.stderr,buf)
    }
    fn flush(&mut self)->std::io::Result<()>{
        if let Ok(mut f)=self.file.lock(){let _=std::io::Write::flush(&mut *f);}
        std::io::Write::flush(&mut self.stderr)
    }
}

fn init_logging(){
    let level=std::env::var("LUCY_LOG_LEVEL").ok().filter(|s|!s.trim().is_empty());
    let log_file=std::env::var("LUCY_LOG_FILE").ok().filter(|s|!s.trim().is_empty());
    let writer:Option<FileMakeWriter>=log_file.and_then(|p|{
        let path=std::path::PathBuf::from(&p);
        if let Some(parent)=path.parent(){
            if !parent.as_os_str().is_empty(){
                if let Err(e)=std::fs::create_dir_all(parent){eprintln!("warning: could not create log dir {}: {e}",parent.display());return None;}
            }
        }
        match std::fs::OpenOptions::new().create(true).append(true).open(&path){
            Ok(f)=>Some(FileMakeWriter{file:Arc::new(std::sync::Mutex::new(f))}),
            Err(e)=>{eprintln!("warning: could not open log file {}: {e}",path.display());None}
        }
    });
    let filter:Option<tracing_subscriber::EnvFilter>=level.map(|l|{
        match tracing_subscriber::EnvFilter::try_new(&l){
            Ok(f)=>Some(f),
            Err(e)=>{eprintln!("warning: invalid LUCY_LOG_LEVEL {l:?}: {e}; using default filter");None}
        }
    }).flatten();
    match (writer,filter){
        (Some(w),Some(f))=>{tracing_subscriber::fmt().with_env_filter(f).with_writer(w).init();}
        (Some(w),None)=>{tracing_subscriber::fmt().with_writer(w).init();}
        (None,Some(f))=>{tracing_subscriber::fmt().with_env_filter(f).with_writer(std::io::stderr).init();}
...
fn sessions_store() -> anyhow::Result<lucy_core::SessionStore> {
    ...
    let dir = cfg.sessions.dir.clone()
        .or_else(|| std::env::var("LUCY_SESSIONS_DIR").ok().map(std::path::PathBuf::from))
...
async fn session_command(args: Vec<String>) -> anyhow::Result<()> {
    ...
    match args.first().map(String::as_str) {
        None | Some("list") | Some("ls") => {
            ...
            if list.is_empty() {
                ...
                for (i, m) in list.iter().enumerate() {
                    println!("{:>2}. {}  [{} msgs]  {}  {}", i + 1, m.title, m.message_count, &m.id.0.to_string()[..8.min(m.id.0.to_string().len())], m.preview.chars().take(60).collect::<String>());
                }
            }
        }
        Some("new") => {
        ...
        Some("show") => {
            let list = store.list().await?;
            let meta = args.get(1).and_then(|a| find_session(&list, a)).or_else(|| list.first().cloned())
        ...
        Some("rename") => {
            let id_arg = args.get(1).ok_or_else(|| anyhow::anyhow!("usage: lucy session rename <id|number> <title>"))?;
            let title = args.get(2..).map(|s| s.join(" ")).filter(|t| !t.trim().is_empty())
                .ok_or_else(|| anyhow::anyhow!("usage: lucy session rename <id|number> <title>"))?;
            let list = store.list().await?;
            let meta = find_session(&list, id_arg).ok_or_else(|| anyhow::anyhow!("no session matches '{id_arg}'"))?;
        ...
        Some("delete") | Some("rm") => {
            let id_arg = args.get(1).ok_or_else(|| anyhow::anyhow!("usage: lucy session delete <id|number>"))?;
            let list = store.list().await?;
            let meta = find_session(&list, id_arg).ok_or_else(|| anyhow::anyhow!("no session matches '{id_arg}'"))?;
        ...
        Some("clear") => {
            let id_arg = args.get(1).ok_or_else(|| anyhow::anyhow!("usage: lucy session clear <id|number>"))?;
            let list = store.list().await?;
            let meta = find_session(&list, id_arg).ok_or_else(|| anyhow::anyhow!("no session matches '{id_arg}'"))?;
            let mut s = store.load(&meta.id).await?;
            s.history.clear();
        ...
        Some("export") => {
            let id_arg = args.get(1).ok_or_else(|| anyhow::anyhow!("usage: lucy session export <id|number> <file.json>"))?;
            let file = args.get(2).ok_or_else(|| anyhow::anyhow!("usage: lucy session export <id|number> <file.json>"))?;
            let list = store.list().await?;
            let meta = find_session(&list, id_arg).ok_or_else(|| anyhow::anyhow!("no session matches '{id_arg}'"))?;
            let s = store.load(&meta.id).await?;
